...
- Artwork image rows are stored in `artwork_images` (not in `artworks` table).
- Physical DB schema includes:
  - `image_url`
  - `image_width`
  - `image_height`
- Current admin API payload/validator contract round-trips:
...
- Artwork detail response returns `images[].image_width` and `images[].image_height`.
- Artwork create/edit UI blocks submit when any image is missing valid dimensions.
...
- Artwork list and course-item thumbnail APIs remain URL-only in this phase:
  - `thumbnail_image_url`
  - no thumbnail dimension fields are exposed yet
