...
- [ ] Artwork 생성/수정 API가 `artwork_images` 기반으로 동작
- [ ] 백오피스 폼에서 다건 이미지 관리 가능
- [ ] `photo_day_url`/`photo_night_url` 코드 참조 제거 완료
- [ ] 코스 아이템 API/프론트 연계 정상
- [ ] 시드/문서 동기화 완료
- [ ] 타입체크/린트/빌드 통과
- [ ] Playwright headed E2E 통과
- [ ] PR 본문 + 기능별 스크린샷 업로드 완료
