...
- [진행 중] 6.4 `next` / 기본 경로 흐름 유지 확인
- [대기] 6.5 운영 배포 기준 회귀 검증
- [대기] 6.6 예외 상황 점검
- [대기] 6.7 마지막 단계: PR 작성
...
확인 포인트:
- `next`가 있으면 원래 요청한 관리자 페이지로 이동하는지
- `next`가 없으면 `/admin/users`로 이동하는지
- `reason=auth-required` alert 흐름이 그대로 동작하는지
...
검증 대상:
- 로컬 개발/빌드 환경
- 운영 배포 `https://steelartdashboard.vercel.app/admin/login`

확인 시나리오:
1. 정상 로그인 후 `/admin/users` 진입
2. 비로그인 접근 후 로그인 페이지 이동 및 원래 경로 복귀
3. 실패 로그인 후 에러 메시지 유지
4. 로그인 직후 더 이상 `/admin/login?reason=auth-required&next=...`로 되돌아가지 않는지 확인
...
단, 현재 관찰상 세션 자체는 생성되므로 1차 수정은 라우팅 방식 변경에 집중한다.
